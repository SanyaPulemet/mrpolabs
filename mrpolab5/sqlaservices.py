...
class BusinessRules:
    def make_order(self, user: int, delivery_address, items, user_repo, order_repo):
        userid = user_repo.get(user).id
        price = sum(300 if isinstance(item, Burger) else 800 for item in items)
        time = datetime.now()
        order = Order(client_id = userid,
                      creation_time = time,
                      delivery_time = None,
...
client1 = Client(name='user1')
client_repository.create(client1)
client2 = Client(name='user2')
...
order1 = Order(creation_time=datetime(year=2024, month=5, day=1), price=1000)
